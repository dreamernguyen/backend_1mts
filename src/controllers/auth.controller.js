...
const {
    promoteGuestUser,
    syncGoogleLogin
} = require('../services/account-link.service');
...
exports.loginWithGoogle = asyncHandler(async (req, res) => {
    ...
    // Tìm theo Firebase UID trước, không khóa theo loginType. Nếu Firebase đã
    // link Google nhưng request /link-google trước đó bị mất mạng, lần đăng nhập
    // này sẽ hoàn tất promotion trên đúng Mongo User cũ thay vì tạo User mới.
    const { user, isNewUser, promoted } = await syncGoogleLogin({
    ...
    return res.status(200).json({
        success: true,
        message: isNewUser
            ? 'Khởi tạo tài khoản Google thành công!'
            : promoted
                ? 'Liên kết Google đã được khôi phục và toàn bộ dữ liệu Khách được giữ nguyên.'
                : 'Chào mừng trở lại!',
...
exports.linkGoogleAccount = asyncHandler(async (req, res) => {
    ...
    // Tìm đúng Mongo User hiện tại. Cho phép loginType=google để request retry
    // sau khi backend đã xử lý thành công vẫn trả kết quả idempotent.
    const guestUser = await User.findById(guestUserId);
    if (!guestUser) {
        return res.status(404).json({ success: false, message: 'Không tìm thấy tài khoản cần liên kết!' });
    }

    const { user, promoted } = await promoteGuestUser({
        user: guestUser,
        firebaseUser
    });
    ...
    return res.status(200).json({
        success: true,
        idempotentReplay: !promoted,
        message: promoted
            ? 'Cập nhật tài khoản sang Google thành công! Toàn bộ dữ liệu được giữ nguyên.'
            : 'Tài khoản đã được liên kết Google trước đó.',
        user
    });
});
...
exports.deleteAccountData = asyncHandler(async (req, res) => {
    ...
    if (!userId || req.user.userId.toString() !== userId.toString()) {
        return res.status(403).json({
            success: false,
    ...
    // Firebase là hệ thống ngoài MongoDB: nếu bước này lỗi thì giữ nguyên dữ liệu
    // nghiệp vụ để người dùng có thể thử lại, tránh trạng thái "xóa nửa chừng".
    try {
